...
for r in [0.0]:
	for err in ["Gaussian"]:
		...
		result_file = open("/data/Coral/" + err + "_historical_coral" + "_" + str(my_buffer) + '_' + nn + ".dat", 'w');	
		result_file.write("Error Patch Central Min Max\n")		
				
		coral = [ ]
		central = [ ];
		for t in range(0,T,skip):
			name = "/data/Coral/Centralheight-" + str(t) + "r" + str(r) + "_" + str(my_buffer) + '_' + nn + "_data.csv"
			print( name )
			...
			for i in range(B):
				if err == "Central":
					name = "/data/Coral/" + err + "height-" + str(t) + "r" + str(r) + "_" + str(my_buffer) + '_' + nn + "_data.csv"
				else:
					name = "/data/Coral/" + err + str(i) + "height-" + str(t) + "r" + str(r) + "_" + str(my_buffer) + '_' + nn + "_data.csv"
				print( name )
				with open(name, 'r') as csvfile:
					spamreader = csv.reader(csvfile, quotechar='"', delimiter=',', quoting=csv.QUOTE_ALL, skipinitialspace=True)
					csum = [ 0 for p in polygons ]
		...
		#plt.ylim([0.98,1.3]);
		#plt.savefig(err + "_historical_coral" + "_" + str(my_buffer) + '_' + nn + ".png")
		plt.show()
		plt.close()
